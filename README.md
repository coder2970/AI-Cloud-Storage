...
cd ~/Project/AI_YunCunChu-main/docker
...
├── CMakeLists.txt              # CMake 构建入口，定义公共库和所有 CGI 目标
├── Makefile                    # 旧版编译规则，保留作为兼容入口
...
├── common/                     # 公共模块
│   ├── cfg.cpp                 #   配置文件解析（读取 cfg.json）
│   ├── make_log.cpp            #   日志模块
│   ├── cpp_cgi.cpp             #   C++ CGI 公共封装（JSON/MySQL/Redis/Token 等）
│   ├── md5.c                   #   MD5 哈希算法实现
...
├── include/                    # 头文件
│   ├── cfg.h / redis_keys.h
│   ├── make_log.h / cpp_cgi.hpp / cgi_constants.h / md5.h / des.h / base64.h
...
| 文件 | 说明 |
|------|------|
| `cfg.cpp` | 读取 `conf/cfg.json` 配置文件，按 section/key 获取值 |
| `make_log.cpp` | 日志记录模块，按日期输出到 `logs/` 目录 |
| `cpp_cgi.cpp` | C++ CGI 公共封装：JSON、配置、MySQL、Redis、Token、响应 JSON |
| `cgi_constants.h` | CGI 响应码和固定长度常量 |
| `md5.c` | MD5 哈希算法 C 实现 |
...
3. **FastCGI 应用容器**：`ubuntu:20.04` 中编译安装 hiredis → libfastcommon → FastDFS → FAISS → 使用 CMake 编译项目 C/C++ 源码
...
| jsoncpp | 系统包 | C++ JSON 解析与响应构造 |

---

## 13. 后端 C++11 重构说明

后端 CGI 主业务模块已经从原来的 C 风格实现迁移到 C++11 实现，Docker 构建当前使用 CMake 编译 `src_cgi/*.cpp` 版本，Makefile 保留作为兼容入口。
...
- 使用 C++11 RAII 管理 JSON、MySQL、Redis 等资源，减少手动释放遗漏。
- 抽取 `include/cpp_cgi.hpp` 和 `common/cpp_cgi.cpp` 作为公共 C++ CGI 工具层。
- 引入 jsoncpp 重构公共 `JsonDoc` / `JsonBuilder`，替换业务层对 cJSON 的直接依赖。
- 统一 JSON 请求读取、token 校验、状态 JSON、配置读取、时间生成、目录创建等公共逻辑。
- 对 SQL 拼接参数做 `mysql_real_escape_string` 转义，降低注入风险。
- 保留 FastCGI + MySQL + Redis + FastDFS + FAISS 的原有架构和接口路径。

当前验证状态：

- Docker 环境下 CMake 构建链路已通过，所有 FastCGI 可执行文件输出到 `bin_cgi/`。
- 在当前 Windows 环境下使用临时第三方头文件桩完成了 C++11 语法级检查。
- 真实完整编译仍需 Docker/Ubuntu 依赖环境，包括 `libfcgi-dev`、`libmysqlclient-dev`、hiredis、FastDFS、FAISS、libcurl 等。
...
如果不使用 Docker，需要在 Ubuntu 环境安装完整依赖后执行：
