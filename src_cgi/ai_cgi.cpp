...
#define AI_PATH_BUF_LEN 1024

static void read_cfg_value(const char *section, const char *key, char *value, size_t value_len)
{
    if (get_cfg_value_len(CFG_PATH, section, key, value, value_len) != 0 && value_len > 0) {
        value[0] = '\0';
    }
}
...
{
    read_cfg_value("mysql", "user", mysql_user, sizeof(mysql_user));
    read_cfg_value("mysql", "password", mysql_pwd, sizeof(mysql_pwd));
    read_cfg_value("mysql", "database", mysql_db, sizeof(mysql_db));

    read_cfg_value("redis", "ip", redis_ip, sizeof(redis_ip));
    read_cfg_value("redis", "port", redis_port, sizeof(redis_port));

    read_cfg_value("dashscope", "embedding_model", embedding_model, sizeof(embedding_model));
    read_cfg_value("dashscope", "vl_model", vl_model, sizeof(vl_model));

    char dim_str[16] = {0};
    read_cfg_value("dashscope", "embedding_dimension", dim_str, sizeof(dim_str));
    if (strlen(dim_str) > 0) embedding_dimension = atoi(dim_str);

    read_cfg_value("faiss", "index_path", faiss_index_path, sizeof(faiss_index_path));
    read_cfg_value("faiss", "user_index_dir", faiss_user_index_dir, sizeof(faiss_user_index_dir));

    if (strlen(faiss_user_index_dir) == 0 && strlen(faiss_index_path) > 0) {
        snprintf(faiss_user_index_dir, sizeof(faiss_user_index_dir), "%s", faiss_index_path);
    ...
    read_cfg_value("web_server", "ip", web_server_ip, sizeof(web_server_ip));
    read_cfg_value("web_server", "port", web_server_port, sizeof(web_server_port));

    read_cfg_value("public_server", "ip", public_server_ip, sizeof(public_server_ip));
    read_cfg_value("public_server", "port", public_server_port, sizeof(public_server_port));
...
static int extract_docx_text(const char *docx_path, char *out_text, int max_len)
{
    ...
    if (pid == 0) {
        ...
        close(pipefd[1]);
        execlp("unzip", "unzip", "-p", docx_path, "word/document.xml", (char *)NULL);
        _exit(1);
    }
...
static bool parse_json_body(const char *body, Json::Value *root)
{
...
static bool get_required_string(const Json::Value &root, const char *key, std::string *out)
{
    if (!key || !out || !root.isMember(key) || !root[key].isString()) return false;
    *out = root[key].asString();
    return true;
}

static void get_optional_string(const Json::Value &root, const char *key, std::string *out)
{
    if (!key || !out || !root.isMember(key) || !root[key].isString()) {
        if (out) out->clear();
        return;
    }
    *out = root[key].asString();
...
static int get_optional_bool(const Json::Value &root, const char *key)
{
    if (!key || !root.isMember(key)) return 0;
    const Json::Value &item = root[key];
    if (item.isBool()) return item.asBool() ? 1 : 0;
    return 0;
}

static bool parse_describe_request(const char *body, DescribeRequest *req, int *invalid_json)
{
    Json::Value root;
    if (invalid_json) *invalid_json = 0;
    if (!req || !parse_json_body(body, &root)) {
        if (invalid_json) *invalid_json = 1;
        return false;
    }

    if (!get_required_string(root, "user", &req->user) ||
        !get_required_string(root, "token", &req->token) ||
        !get_required_string(root, "md5", &req->md5) ||
        !get_required_string(root, "filename", &req->filename)) {
        return false;
    }

    get_optional_string(root, "type", &req->type);
    get_optional_string(root, "api_key", &req->api_key);
    req->force_update = get_optional_bool(root, "force");
    req->skip_rebuild = get_optional_bool(root, "skip_rebuild");
    return true;
}

static bool parse_search_request(const char *body, SearchRequest *req, int *invalid_json)
{
    Json::Value root;
    if (invalid_json) *invalid_json = 0;
    if (!req || !parse_json_body(body, &root)) {
    ...
    if (!get_required_string(root, "user", &req->user) ||
        !get_required_string(root, "token", &req->token) ||
        !get_required_string(root, "query", &req->query)) {
        return false;
    }
...
static bool parse_rebuild_request(const char *body, RebuildRequest *req, int *invalid_json)
{
    Json::Value root;
    if (invalid_json) *invalid_json = 0;
    if (!req || !parse_json_body(body, &root)) {
    ...
    return get_required_string(root, "user", &req->user) &&
           get_required_string(root, "token", &req->token);
}

static void print_json_value(const Json::Value &value)
{
    Json::StreamWriterBuilder builder;
    std::string text;
    builder["indentation"] = "";
    text = Json::writeString(builder, value);
    FCGI_printf("%s\n", text.c_str());
}
...
{
    char dirty_path[AI_PATH_BUF_LEN] = {0};
...
{
    char lock_path[AI_PATH_BUF_LEN] = {0};
    int fd = -1;
...
static int query_one_field(MYSQL *conn, const char *sql_cmd, char *buf, size_t buf_len)
{
    if (!conn || !sql_cmd) return -1;
    if (mysql_query(conn, sql_cmd) != 0) {
        LOG(AI_LOG_MODULE, AI_LOG_PROC, "mysql query failed: %s, sql=%s\n", mysql_error(conn), sql_cmd);
        return -1;
    }

    ycc::MysqlResult result(mysql_store_result(conn));
    if (!result.get()) {
        LOG(AI_LOG_MODULE, AI_LOG_PROC, "mysql_store_result failed: %s, sql=%s\n", mysql_error(conn), sql_cmd);
        return -1;
    }

    MYSQL_ROW row = mysql_fetch_row(result.get());
    if (!row) {
        if (buf && buf_len > 0) snprintf(buf, buf_len, "%s", "0");
        return 1;
    }

    if (!buf || buf_len == 0) return 2;
    snprintf(buf, buf_len, "%s", row[0] ? row[0] : "");
    return 0;
}
...
static int user_owns_file(MYSQL *conn, const char *user, const char *md5, char *file_name, int file_name_len)
{
    ...
    if (file_name && file_name_len > 0) {
        ret = query_one_field(conn, sql, file_name, (size_t)file_name_len);
    } else {
        ret = query_one_field(conn, sql, NULL, 0);
    }
...
static int get_user_ai_status(MYSQL *conn, const char *user, const char *md5, int *status_out)
{
    ...
    snprintf(sql, sizeof(sql),
             ...
             escaped_user, escaped_md5);
    ret = query_one_field(conn, sql, tmp, sizeof(tmp));
    if (ret == 0 && status_out) {
        *status_out = atoi(tmp);
    }
                                         ...
                                         char *description, int description_len)
{
    ...
    if (is_image_type(type)) {
        ...
        if (query_one_field(conn, sql_cmd, db_url, sizeof(db_url)) == 0 && strlen(db_url) > 0) {
...
static int handle_describe(char *post_data)
{
    ...
    if (!parse_describe_request(post_data, &req, &invalid_json)) {
        if (invalid_json) {
            LOG(AI_LOG_MODULE, AI_LOG_PROC, "parse describe request failed\n");
            FCGI_printf("{\"code\":1,\"msg\":\"invalid json\"}\n");
            return -1;
        }
        LOG(AI_LOG_MODULE, AI_LOG_PROC, "describe missing required fields\n");
        FCGI_printf("{\"code\":1,\"msg\":\"missing fields\"}\n");
        return -1;
    }

    // 仅接受请求体中的 api_key
    const char *api_key = req.api_key.c_str();
    if (!api_key || strlen(api_key) == 0) {
        FCGI_printf("{\"code\":1,\"msg\":\"missing api_key\"}\n");
        return -1;
    }
    const char *user = req.user.c_str();
    const char *token = req.token.c_str();
    const char *md5 = req.md5.c_str();
    const char *filename = req.filename.c_str();
    const char *type = req.type.c_str();

    // 验证 token
    ret = ycc::verify_token(user, token) ? 0 : -1;
    if (ret != 0) {
        FCGI_printf("{\"code\":4,\"msg\":\"token error\"}\n");
        return -1;
    }
    ...
    if (!conn) {
        ...
        FCGI_printf("{\"code\":1,\"msg\":\"db error\"}\n");
        return -1;
    }

    // 校验当前用户是否真正拥有该文件
    if (user_owns_file(conn, user, md5, owner_file_name, sizeof(owner_file_name)) != 0) {
        FCGI_printf("{\"code\":1,\"msg\":\"file not found or no permission\"}\n");
        goto END;
    }
    ...
    // 如果当前用户已有完成记录，直接返回
    {
        ...
        if (user_ai_ret == 0 && user_ai_status == 1 && !req.force_update) {
            FCGI_printf("{\"code\":0,\"msg\":\"already exists\"}\n");
            ret = 0;
            goto END;
        }
    }
    ...
    if (generate_description_for_file(conn, md5,
        ...
        FCGI_printf("{\"code\":1,\"msg\":\"describe failed\"}\n");
        goto END;
    }
    ...
    if (!vector) {
        ...
        FCGI_printf("{\"code\":1,\"msg\":\"memory error\"}\n");
        goto END;
    }
    ...
    if (ret != 0) {
        ...
        FCGI_printf("{\"code\":1,\"msg\":\"embedding failed\"}\n");
        goto END;
    }
    ...
    if (replace_global_ai_cache(conn, md5, description, vector, embedding_model, 1) != 0) {
        FCGI_printf("{\"code\":1,\"msg\":\"cache update failed\"}\n");
        goto END;
    }
    ...
    if (replace_user_ai_record(conn, user, md5,
        ...
        FCGI_printf("{\"code\":1,\"msg\":\"user ai update failed\"}\n");
        goto END;
    }

    if (req.force_update) {
        if (!req.skip_rebuild && rebuild_user_faiss_index(conn, user) < 0) {
            FCGI_printf("{\"code\":1,\"msg\":\"rebuild failed\"}\n");
            goto END;
        }
...
static int handle_search(char *post_data)
{
    ...
    if (!parse_search_request(post_data, &req, &invalid_json)) {
        FCGI_printf(invalid_json ? "{\"code\":1,\"msg\":\"invalid json\"}\n"
                                 : "{\"code\":1,\"msg\":\"missing fields\"}\n");
        return -1;
    }

    const char *user = req.user.c_str();
    const char *token = req.token.c_str();
    const char *query = req.query.c_str();
    ...
    if (strlen(query) == 0) {
        FCGI_printf("{\"code\":1,\"msg\":\"empty query\"}\n");
        return -1;
    }
...

    {
        ...
        if (sync_ret < 0) {
            ...
            FCGI_printf("{\"code\":1,\"msg\":\"sync failed\"}\n");
            goto END;
        }
        if (sync_ret > 0) {
    ...
    if (lock_fd < 0) {
        FCGI_printf("{\"code\":1,\"msg\":\"lock error\"}\n");
        goto END;
    }

    if (faiss_init(index_path, embedding_dimension) != 0) {
        FCGI_printf("{\"code\":1,\"msg\":\"index init failed\"}\n");
        goto END;
    }
    ...
    if (faiss_get_ntotal() == 0) {
        ...
        snprintf(ready_sql, sizeof(ready_sql),
                 ...
                 escaped_user);
        if (query_one_field(conn, ready_sql, ready_count, sizeof(ready_count)) == 0 && atoi(ready_count) > 0) {
            release_user_lock(lock_fd);
            lock_fd = -1;
    ...
    if (faiss_get_ntotal() == 0) {
        FCGI_printf("{\"code\":0,\"count\":0,\"files\":[]}\n");
        ret = 0;
        goto END;
    }
    ...
    // FAISS 搜索
    {
        ...
        for (int i = 0; i < found; i++) {
            ...
            if (row) {
                ...
                file_obj["filename"] = row[1] ? row[1] : "";
                file_obj["description"] = row[2] ? row[2] : "";
                ...
                file_obj["size"] = row[4] ? row[4] : "0";
                file_obj["type"] = row[5] ? row[5] : "";
                file_obj["score"] = scores[i];
...
static int handle_rebuild(char *post_data)
{
    ...
    if (!parse_rebuild_request(post_data, &req, &invalid_json)) {
        FCGI_printf(invalid_json ? "{\"code\":1,\"msg\":\"invalid json\"}\n"
                                 : "{\"code\":1,\"msg\":\"missing fields\"}\n");
        return -1;
    }

    ret = ycc::verify_token(req.user.c_str(), req.token.c_str()) ? 0 : -1;
    if (ret != 0) {
        FCGI_printf("{\"code\":4,\"msg\":\"token error\"}\n");
        return -1;
    }
...

    {
        ...
        FCGI_printf("{\"code\":0,\"msg\":\"rebuilt\",\"count\":%d}\n", rebuild_count);
        ret = 0;
    }
...
int main()
{
    ...
    while (FCGI_Accept() >= 0) {
        ...
        char cmd[64] = {0};
        if (query_string) {
            std::string cmd_value = ycc::query_param(query_string, "cmd");
            snprintf(cmd, sizeof(cmd), "%s", cmd_value.c_str());
        }

        if (strlen(cmd) == 0) {
            FCGI_printf("{\"code\":1,\"msg\":\"missing cmd\"}\n");
            continue;
        }
        ...
        if (content_length > 0 && content_length < 1024 * 1024) {
            ...
            if (post_data) {
                int bytes_read = FCGI_fread(post_data, 1, content_length, FCGI_stdin);
                post_data[bytes_read] = '\0';
            }
        }

        if (!post_data || strlen(post_data) == 0) {
            FCGI_printf("{\"code\":1,\"msg\":\"no post data\"}\n");
        ...
        if (strcmp(cmd, "describe") == 0) {
            ...
            FCGI_printf("{\"code\":1,\"msg\":\"unknown cmd: %s\"}\n", cmd);
        }
